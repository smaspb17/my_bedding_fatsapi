...
class S3MinioClient:
    def __init__(
        ...
        self.config = {
            ...
            "use_ssl": False  # MinIO работает по HTTP для локального хранилища
        }
    ...
    async def upload_file(
        ...
        try:
            ...
            logger.info(f"Файл {file_location} загружен в MinIO ({self.bucket_name})")
    ...
    async def delete_one_file(self, file_name: str):
        ...
        try:
            async with self.get_client() as client:
                ...
                logger.info(f"Файл {file_name} удален из MinIO ({self.bucket_name})")
        except (ClientError, EndpointConnectionError) as e:
    ...
    async def delete_all_files(self, file_names: list[str], client):
        ...
        try:
            for file_name in file_names:
                await client.delete_object(Bucket=self.bucket_name, Key=file_name)
            logger.info(f"Удалены файлы {file_names} из MinIO ({self.bucket_name})")
        except (ClientError, EndpointConnectionError) as e:
...
class S3SelectelClient:
    ...
    async def upload_file(
        ...
        try:
            ...
            print(f"Файл {file_location} загружен в хранилище {self.bucket_name}")
    ...
    async def delete_one_file(
        ...
        """Удаление одного файла у товара из S3"""
        try:
            async with self.get_client() as client:
                await client.delete_object(Bucket=self.bucket_name, Key=file_name)
                print(f"Файл {file_name} удален из {self.bucket_name}")
        except ClientError as e:
            print(f"Ошибка удаления файла в S3: {e}")
        except Exception as e:
    ...
    async def delete_all_files(
        ...
        """Удаление всех файлов у товара из S3"""
        try:
            for file_name in file_names:
                await client.delete_object(Bucket=self.bucket_name, Key=file_name)
        except ClientError as e:
